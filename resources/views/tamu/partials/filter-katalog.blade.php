<div class="flex flex-col justify-between gap-3 md:flex-row md:items-end">
    ...
    <form method="GET" action="{{ route('katalog') }}" class="grid w-full gap-2 md:w-[680px] md:grid-cols-[1.6fr_1fr_1fr_auto_auto]">
        <input
            name="q"
            value="{{ $q ?? '' }}"
        ...
        <a href="{{ route('katalog') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-center text-sm font-semibold text-slate-700 hover:bg-slate-50">
            Reset
        </a>
    </form>
</div>
